...
-- gh-2617 SQL DDL changes
cn:execute('create table test2(id primary key, a unique, b, c)')
-- rowcount = 3 = _space:insert(test2), _index:insert(pk, unique).
cn:reload_schema()
cn:execute('insert into test2 values (1, 2, 3, 4)')
cn:execute('create table if not exists test2(id primary key)')
-- rowcount = 0, table exists.
cn:reload_schema()
box.space.test2:truncate() -- create a tuple in _truncate.
cn:reload_schema()
cn:execute('create index sec_c on test2(c)')
-- rowcount = 1, _index:insert(sec_c)
cn:reload_schema()
cn:execute('drop table test2')
-- rowcount = 5: _space:delete(test2),
--               _index:delete(pk, unique, sec_c),
--               _truncate:delete(test2)
cn:reload_schema()
